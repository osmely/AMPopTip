...
open class PopTip: UIView {
  ...
  @objc open dynamic var contentCornerRadius = CGFloat(4.0)
  ...
  internal func setupVertically() -> (CGRect, CGPoint) {
    ...
    let constrainedArrowOffset = abs(arrowOffset) > (frame.size.width / 2) ? ((arrowOffset / arrowOffset) * (frame.size.width / 2 - contentCornerRadius * 2)) : arrowOffset
  ...
  internal func setupHorizontally() -> (CGRect, CGPoint) {
    ...
    // Constraint the offset in the boundaries of the bubble, maintaining the sign (hence the arrowOffset / arrowOffset)
    let constrainedArrowOffset = abs(arrowOffset) > (frame.size.height / 2) ? ((arrowOffset / arrowOffset) * (frame.size.height / 2  - contentCornerRadius * 2)) : arrowOffset
    // Make sure that the bubble doesn't leave the boundaries of the view
  ...
  open override func draw(_ rect: CGRect) {
    if isRounded {
      let showHorizontally = direction == .left || direction == .right
        contentCornerRadius = (frame.size.height - (showHorizontally ? 0 : arrowSize.height)) / 2
    }

    let path = PopTip.pathWith(rect: rect, frame: frame, direction: direction, arrowSize: arrowSize, arrowPosition: arrowPosition, arrowRadius: arrowRadius, borderWidth: borderWidth, radius: contentCornerRadius)
  ...
  fileprivate func updateBubble() {
    stopActionAnimation {
      UIView.animate(withDuration: 0.2, delay: 0, options: [.transitionCrossDissolve, .beginFromCurrentState], animations: {
        ...
        let path = PopTip.pathWith(rect: self.frame, frame: self.frame, direction: self.direction, arrowSize: self.arrowSize, arrowPosition: self.arrowPosition, arrowRadius: self.arrowRadius, borderWidth: self.borderWidth, radius: self.contentCornerRadius)
